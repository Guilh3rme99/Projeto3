...
void Rex::initializeGL(GLuint program) {
  m_program = program;
  // Pegamos do shader a matriz modelo e normal para renderizarmos o modelo Rex
  modelMatrixRex = abcg::glGetUniformLocation(m_program, "modelMatrix");
  normalMatrixRex = abcg::glGetUniformLocation(m_program, "normalMatrix");
  // Generate VBO of Rex
  ...
  // Processo de criação do VAO do Rex
  // Create VAO
  ...
  // Pegamos os atributos da normal
  ...
  // Metal gear Rex(posição 1)
  modelMatrix[0] = glm::mat4{1.0f};
  modelMatrix[0] = glm::translate(modelMatrix[0], glm::vec3(-1.3, -3.0f, 5.0f));
  modelMatrix[0] = glm::rotate(modelMatrix[0], glm::radians(270.0f), glm::vec3(0, 1, 0));
  modelMatrix[0] = glm::scale(modelMatrix[0], glm::vec3(7.0f));
  // Metal Gear Rex(posição 2)
  modelMatrix[1] = glm::mat4{1.0f};
  modelMatrix[1] = glm::translate(modelMatrix[1], glm::vec3(-1.3f, -3.0f, -2.5f));
  modelMatrix[1] = glm::rotate(modelMatrix[1], glm::radians(270.0f), glm::vec3(0, 1, 0));
  modelMatrix[1] = glm::scale(modelMatrix[1], glm::vec3(7.0f));
}

void Rex::loadDiffuseTexture(std::string_view path) {
...
void Rex::loadObj(std::string_view path, bool standardize) {
...
void Rex::paintGL(glm::mat4 m_viewMatrix) {
  // Bind vertex attributes to current VAO(Metal Gear Rex)
  abcg::glBindVertexArray(m_VAO);
  // Desenhar o rex na posição 1
   auto modelViewMatrix{glm::mat3(m_viewMatrix * modelMatrix[0])};
  glm::mat3 normalMatrix{glm::inverseTranspose(modelViewMatrix)};
  abcg::glUniformMatrix3fv(normalMatrixRex, 1, GL_FALSE, &normalMatrix[0][0]);

  abcg::glUniformMatrix4fv(modelMatrixRex, 1, GL_FALSE,
                           &(modelMatrix[0])[0][0]);
  ...
  // Agora desenhamos outro rex, só que na posição 2
  modelViewMatrix = glm::mat3(m_viewMatrix * modelMatrix[1]);
