...
void Ship::initializeGL(GLuint program) {
  ...
  m_program = program;
  // Pegamos do shader a matriz modelo e normal para renderizarmos o modelo
  // porta-aviões
  modelMatrixShip = abcg::glGetUniformLocation(m_program, "modelMatrix");
  normalMatrixShip = abcg::glGetUniformLocation(m_program, "normalMatrix");
  C = abcg::glGetUniformLocation(m_program, "C");
  // Generate VBO of Porta
  ...
  // Processo de criação do VAO do Porta
  // Create VAO
  ...
  // Agora vamos colocar onde queremos que nosso porta-aviões esteja
  //glm::mat4 modelMatrix{1.0f};
  modelMatrix = glm::translate(modelMatrix, glm::vec3(-1.0f, 5.3f, 0.0f));
  modelMatrix =
      glm::rotate(modelMatrix, glm::radians(90.0f), glm::vec3(0, 1, 0));
  modelMatrix = glm::scale(modelMatrix, glm::vec3(200.0f));
}

void Ship::loadFromFile(std::string_view path, bool standardize) {
...
void Ship::paintGL(glm::mat4 m_viewMatrix) {
  // Bind vertex attributes to current VAO(Porta Aviões)
  abcg::glBindVertexArray(m_VAO);

  auto modelViewMatrix{glm::mat3(m_viewMatrix * modelMatrix)};
  glm::mat3 normalMatrix{glm::inverseTranspose(modelViewMatrix)};
  abcg::glUniformMatrix3fv(normalMatrixShip, 1, GL_FALSE, &normalMatrix[0][0]);

  abcg::glUniformMatrix4fv(modelMatrixShip, 1, GL_FALSE, &modelMatrix[0][0]);

  abcg::glUniform4f(C, 0.5f, 0.5f, 0.5f, 1.0f);
